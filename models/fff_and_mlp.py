...
# Modified to add a (small) MLP to the FFF layer
...
class FFFAndMLP(nn.Module):
	def __init__(self, input_width, output_width, depth, parallel_size, mlp_size, activation=nn.GELU):
		super().__init__()
		...
		self.dense_in = nn.Linear(input_width, mlp_size, bias=False)
		self.dense_out = nn.Linear(mlp_size, output_width, bias=False)
	...
	def forward(self, oldx: torch.Tensor) -> torch.Tensor:
		...
		with torch.no_grad():
			...
			decision_map = torch.zeros_like(decisions, dtype=x.dtype) # (batch_size, parallel_size, n_nodes)
		...
		new_logits += self.dense_out(self.activation(self.dense_in(x)))
