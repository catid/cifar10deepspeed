...
# Modified to add a small MLP that is not conditional
...
class Bojan_Flat_AndMLP_FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, mlp_size, dropout = 0.):
        super().__init__()
        self.net = nn.Sequential(
        ...
        self.dense = FeedForward(dim, mlp_size, dropout)
    def forward(self, x):
        return self.net(x) + self.dense(x)
